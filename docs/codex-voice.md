...
The currently selected session is omitted from the session choices, and session
switching is unavailable while capture/transcription is busy. Status is refreshed
...
`home/user/codex-voice.nix`. It matches the managed COSMIC Catppuccin
