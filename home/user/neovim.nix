...
let
  ...
  customPlugins = {
    nvim-treesitter-with-parsers = vimPlugins.nvim-treesitter.withPlugins (
    ...
    herdr-navigator-nvim = buildPlugin {
  ...
  pluginMap = {
    ...
    "nvim-treesitter/nvim-treesitter" = customPlugins.nvim-treesitter-with-parsers;
  };
...
in
{
  programs.neovim = {
    ...
    vimAlias = true;
    vimdiffAlias = true;
    withNodeJs = config.dotfiles.capabilities.development;
    withPython3 = true;
    withRuby = false;
    ...
    # Keep weekly/daily tools on the wrap PATH. Rare/never tools (ccls,
    # bashls, lua_ls, marksman, eslint, prettier, stylua, nodejs_latest)
    # live in project shells when needed — ccls alone was ~1.7 GiB via
    # clang/llvm.
    extraPackages =
      with pkgs;
      [
      ...
      ++ lib.optionals config.dotfiles.capabilities.development [
        black
        gopls
        isort
        pyright
      ...
      ++ lib.optionals config.dotfiles.capabilities.work [ terraform-ls ]
      ++ lib.optionals config.dotfiles.capabilities.desktop [ xdg-utils ]
      ++ lib.optionals (pkgs.stdenv.isLinux && config.dotfiles.capabilities.desktop) [
        wl-clipboard
        xclip
      ];
  };
