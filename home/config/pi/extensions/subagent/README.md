...
A child calls `ask_coordinator` when it needs clarification. Its execution segment ends with a typed question, not a completed task. The coordinator receives the first settled question without waiting for slow siblings, while the job retains running and queued work.

The coordinator uses `team answer` for an evidence-backed answer, or `team ask` to display the question in this main pane. Questions marked `requiresUser` accept answers only through actual main-pane UI input or a user slash-answer, never a model-supplied provenance flag. Dismissing or cancelling the dialog leaves the question pending.
