...
import { registerTeamControls } from "./controls.js";
...
const ChainItem = Type.Object({
	skills: Type.Optional(Type.Array(Type.String(), { description: "Additional harness skills for this step" })),
	agent: Type.String({ description: "Name of the agent to invoke" }),
	...
	const registerSubagent: typeof pi.registerTool = (tool) => pi.registerTool({
		...
		async execute(...args) {
			...
			if ((result as any).isError) {
				throw new Error(result.content.filter((part) => part.type === "text").map((part) => (part as any).text).join("\n"));
			}
			return result;
		},
	});
	registerSubagent({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			if (process.env.PI_TEAM_CHILD === "1") {
				return { content: [{ type: "text", text: "Team children return requests to their coordinator; recursive delegation is disabled." }], isError: true };
			}
