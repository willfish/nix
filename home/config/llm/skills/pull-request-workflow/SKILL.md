...
- Follow the repository's PR template, but omit verification fields unless the
  user explicitly requests them. Do not invent additional top-level sections.
  Keep all verification details out of PR bodies, including test commands,
  results, CI status and manual evidence. Report checks and blockers in the
  conversation instead; still perform all required verification.
