...
Checked: 2026-06-20
Update trigger: improve workflow change, executor handoff failure, or quarterly review.
...
File naming: `plans/NNN-short-slug.md`, numbered in recommended execution order. In this dotfiles harness, `plans/` is local gitignored scratch space. Do not add or commit plan/spec files.
...
> in `plans/README.md` only if the plan index is being maintained locally in
> this worktree — unless a reviewer dispatched you and told you they maintain
> the index.
>
...
- [ ] `plans/README.md` status row updated when a local plan index is being maintained
...
The index is local scratch state. Do not add or commit it. It is useful for
multi-plan advisor sessions, but it is not a required source artifact for
