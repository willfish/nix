...
If the host exposes isolated executor/subagent tooling, spawn **one** executor in a disposable git worktree. Use the executor model or profile named by the user if they named one; otherwise use the host default for implementation work. If isolated execution is not available, stop and hand the plan to the user for manual execution instead of editing the main worktree yourself.
