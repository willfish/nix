...
**Verification is work you must do, not boilerplate you must publish.** Run the
required checks and inspect their results. Do not add “CI green”, “all tests
pass”, build logs, routine test counts, or a verification checklist to PRs when
CI already supplies that information. If a template requires a test field,
refer to CI briefly rather than duplicating its results.

Include verification evidence only when it adds something reviewers need that
CI does not provide: a manual UI demonstration, a meaningful before/after
benchmark, or a coverage gap that changes merge risk. State the observation and
its limits, not the testing journey. Keep full execution logs in the session or
appropriate evidence store. Report required completion evidence to the requester
without copying it into commits, PRs, or architectural docs.
...
- PR, useful evidence: “Keyboard focus returns to the trigger after closing the
  dialog; checked manually because the browser suite does not cover focus.”
...
- [Simon Willison: Your job is to deliver code you have proven to work](https://simonwillison.net/2025/Dec/18/code-proven-to-work/):
  verification must not be offloaded to reviewers. The no-duplicate-CI rule above
  is our editorial policy, not his prescription to omit evidence.
