...
- Includes specific evidence only when it explains the mechanism or affects review: relevant version constraints or failure probabilities, not routine test counts or CI status
...
- "I think we should socialise that question with the responsible maintainers"
...
Good: `fix(specs): stabilise category assessment exemption coverage`
Good: `chore(logging): add silence_healthcheck_path for rack healthchecks`
Bad: `PROJ-123: Fixed some issues with the spec`
