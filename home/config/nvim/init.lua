...
-- Copy previous line indent on <CR>/o/O when indentexpr returns -1 or is unset.
...
vim.keymap.set(
	...
	"<Leader>o",
	":GBrowse<CR>",
	{ desc = "Open the current buffer (or selection) in a browser" }
)
...
require("blink.cmp").setup({
	keymap = {
		...
		-- Restore nvim-cmp/LuaSnip muscle memory: jump in an active snippet, else
		-- accept the selected completion item (expands snippet items).
...
-- LSPs whose packages live on the Home Manager neovim wrap PATH.
-- bashls, ccls, eslint, lua_ls, marksman are omitted (rare/never); enable
-- via project tooling if needed.
...
require("conform").setup({
	...
	-- prettier/stylua live in project shells when needed (not on the neovim
	-- wrap PATH). isort + black stay on the wrap PATH for Python formatting.
	-- Outside ~/.dotfiles, format_on_save falls back to LSP after formatters.
	formatters_by_ft = {
		fish = { "treefmt_dotfiles", "fish_indent", stop_after_first = true },
		javascript = { "treefmt_dotfiles" },
		json = { "treefmt_dotfiles" },
		lua = { "treefmt_dotfiles" },
