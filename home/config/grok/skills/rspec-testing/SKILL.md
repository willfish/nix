...
- `references/rspec.md` — Better Specs-derived checklist and syntax conventions
  (describe `.method` / `#method`, contexts starting with "when/with/without",
  short descriptions, one behavior per example, valid/edge/invalid cases,
  observable behavior, external HTTP stubbing, `aggregate_failures`, `let` over
  `before`, etc.).
