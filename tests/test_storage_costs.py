...
class StorageCostsTest(unittest.TestCase):
    def setUp(self):
        ...
        self.tool(
            ...
            python3 -c '
import json, os, sys
open(os.environ["STORAGE_CALLS"], "a").write(
    json.dumps({"tool": "nh", "args": sys.argv[1:]}) + "\\n"
)
' "$@"
...
with open(os.environ["STORAGE_CALLS"], "a") as log:
    log.write(json.dumps({
        "tool": "nix", "args": args,
        "flake": os.environ.get("NIX_STORAGE_COST_FLAKE"),
        "scope": os.environ.get("NIX_STORAGE_COST_SCOPE"),
    }) + "\\n")
if args[0] == "build":
    print("/nix/store/fixture-home"
          if any("homeConfigurations" in a for a in args)
          else "/nix/store/fixture-system")
elif args[0] == "eval":
    print("[]" if "--json" in args else
          os.environ.get("STORAGE_SELECTED_HOME", "william-linux"))
elif args[0] == "path-info":
    print(json.dumps({a: {"narSize": 5, "closureSize": 7}
                      for a in args if a.startswith("/nix/store/")}))
else:
...
PY
    ...
    def test_home_scope_never_builds_nixos(self):
        ...
        self.assertNotIn(
            "NixOS system:",
            next((self.root / "state").rglob("*.md")).read_text(),
        )
    ...
    def test_system_scope_never_builds_home(self):
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assert_one_build(
            '#nixosConfigurations."foundation".config.system.build.toplevel'
        )
    ...
    def test_nh_system_hostname_flag_and_environment_flake(self):
        ...
        result = self.run_report(
            "--after-nh-switch", "os", "switch", "--hostname=foundation"
        )
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assert_one_build(
            '#nixosConfigurations."foundation".config.system.build.toplevel'
    ...
    def test_nh_home_environment_flake_has_precedence(self):
        ...
        result = self.run_report(
            "--after-nh-switch", "home", "switch", "-c", "william-linux"
        )
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assert_one_build(
            '#homeConfigurations."william-linux".activationPackage'
        )
    ...
    def test_dry_and_help_callbacks_never_call_nix(self):
        for flag in ("--dry", "-n", "--help", "-h"):
            with self.subTest(flag=flag):
                result = self.run_report(
                    "--after-nh-switch", "os", "switch", str(self.flake), flag
                )
                self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_store_result_symlink_callback_is_skipped_after_resolution(self):
        ...
        result = self.run_report(
            "--after-nh-switch", "os", "switch", str(result_link)
        )
    ...
    def test_direct_linux_report_keeps_both_scopes(self):
        result = self.run_report(
            "--flake", str(self.flake), "--home", "william-linux"
        )
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(len(self.builds()), 2)
        self.assertTrue(
            all(call["flake"] == str(self.flake) for call in self.builds())
        )
    ...
    def test_inventory_does_not_evaluate_the_unselected_configuration(self):
        ...
        library = """rec {
          ...
          attrByPath = path: fallback: attrs:
            ...
            then attrByPath (builtins.tail path) fallback
              attrs.${builtins.head path}
            else fallback;
        }"""
        home = """{ fixture.config = {
          home.packages = [];
        ...
        for scope in ("home", "system"):
            with self.subTest(scope=scope):
                home_config = (
                    home
                    if scope == "home"
                    else 'throw "evaluated unselected home"'
                )
                system_config = (
                    system
                    if scope == "system"
                    else 'throw "evaluated unselected system"'
                )
                expression = (
    ...
    def run_shell(self, shell, args):
        ...
        command = (
            'source "$1"; shift; nh "$@"'
            if shell == "bash"
            else "source $argv[1]; nh $argv[2..-1]"
        )
        cmd = (
            [shell, "--noprofile", "--norc"]
            if shell == "bash"
            else [shell, "--no-config"]
        )
        cmd += ["-c", command]
        if shell == "bash":
            cmd += ["fixture"]
    ...
    def test_bash_and_fish_preserve_nh_arguments_and_report_selected_flake(
        self,
    ):
        args = [
            "home",
        ...
        for shell in ("bash", "fish"):
            with self.subTest(shell=shell):
                result = self.run_shell(
                    shell, ["home", "switch", str(self.flake)]
                )
    ...
    def test_report_failure_does_not_change_successful_nh_status(self):
        ...
        for shell in ("bash", "fish"):
            with self.subTest(shell=shell):
                result = self.run_shell(
                    shell,
                    ["home", "switch", str(self.flake), "-c", "william-linux"],
                )
                self.assertEqual(result.returncode, 0, result.stderr)
