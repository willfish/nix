...
if (existsSync(extensionPath)) {
  ...
  test('managed root emits reply and accepts dictation', async t => {
    const f = await managed(t, { env: { PI_TEAM_CHILD: '0' } });
  ...
  test('team children never attach or accept dictation', async t => {
    const hooks = {};
    registerVoice({ on: (name, fn) => { hooks[name] = fn; } }, {
      HERDR_ENV: '1', HERDR_PANE_ID: 'w1:p2', HERDR_SOCKET_PATH: '/tmp/herdr.sock',
      XDG_RUNTIME_DIR: '/run/user/test', PI_TEAM_CHILD: '1',
    });
    assert.equal(hooks.session_start, undefined);
  });

  test('managed TUI binds a private UUID endpoint and acknowledges operational readiness', async t => {
    const f = await managed(t, { env: { PI_TEAM_CHILD: '0', PI_VOICE_HARNESS: 'qwen-pi' } });
    await until(() => f.scheduler.timers.size);
    ...
    assert.equal(target.team_child, false);
  ...
  test('RPC, print and JSON children inheriting Herdr never create a bridge', async t => {
    for (const mode of ['rpc', 'print', 'json']) {
      const f = await managed(t, { mode, env: { PI_TEAM_CHILD: '0' } });
