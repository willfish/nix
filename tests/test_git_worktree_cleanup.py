"""Exercise the installed Git wrapper against disposable real Git repositories."""
...
class GitWorktreeCleanupTest(unittest.TestCase):
    def setUp(self):
        ...
        # Only child processes see this home and Git config. Never use real state.
        self.env.update(
            HOME=str(self.home),
        ...
        exec_path = Path(subprocess.check_output(
        ...
        self.assertEqual(len(matches), 1, "extract exactly the installed wrapper")
        ...
        # The socket namespace is also isolated, even on accidental ID collision.
        ...
        self.real("init", "--initial-branch=main", str(self.repo), cwd=self.root)
    ...
    def real(self, *args, cwd=None):
        return subprocess.run(
            [str(self.git), *args], cwd=cwd or self.repo, env=self.env,
            text=True, capture_output=True, check=True,
        )

    def wrapped(self, *args, cwd=None):
        return subprocess.run(
            [str(self.wrapper), *args], cwd=cwd or self.repo, env=self.env,
            text=True, capture_output=True, timeout=15,
        )

    def worktree(self, name="feature", locked=False, missing=False):
        path = self.root / name
        self.real("worktree", "add", "-b", "branch-" + re.sub(r"\s", "-", name), str(path))
        if locked:
            self.real("worktree", "lock", "--reason", "retain fixture", str(path))
        ...
        (state / "postmaster.pid").write_text("synthetic PID, never read by real tools\n")
        if missing:
            shutil.rmtree(path)
        return path, state
    ...
    def test_untracked_remove_never_cleans_state(self):
        ...
        self.assertNotEqual(self.wrapped("worktree", "remove", str(path)).returncode, 0)
        self.assert_state_retained(state)

    def test_invalid_remove_option_never_cleans_state(self):
        path, state = self.worktree()
        result = self.wrapped("worktree", "remove", "--invalid-option", str(path))
        self.assertNotEqual(result.returncode, 0)
        self.assert_state_retained(state)
    ...
    def test_successful_remove_resolves_basename_with_global_directory_option(self):
        path, state = self.worktree("feature with spaces")
        result = self.wrapped("-C", str(self.repo), "worktree", "remove", path.name, cwd=self.root)
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertFalse(state.exists())
    ...
    def test_failed_postgres_stop_preserves_state(self):
        ...
        result = self.wrapped("worktree", "remove", str(path))
        self.assertNotEqual(result.returncode, 0)
        self.assertFalse(path.exists(), "Git accepted removal before cleanup failed")
        self.assertTrue((state / "sentinel").is_file())
    ...
    def test_locked_missing_worktree_survives_prune(self):
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn(str(path), self.real("worktree", "list", "--porcelain").stdout)
        self.assert_state_retained(state)
    ...
    def test_default_prune_matches_real_git(self):
        ...
        removed = str(reference) not in self.real("worktree", "list", "--porcelain").stdout
    ...
    def test_prune_dry_run_preserves_state(self):
        _, state = self.worktree(missing=True)
        result = self.wrapped("worktree", "prune", "--dry-run", "--expire", "now")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assert_state_retained(state)
