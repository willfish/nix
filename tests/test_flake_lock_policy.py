"""Exercise the privileged workflow's policy step with local lock-file
fixtures.
"""
...
class FlakeLockPolicyTest(unittest.TestCase):
    def setUp(self):
        ...
        gh.write_text(
            ...
            if args[:2] == ["pr", "view"]:
                if args[3:] == ["--json", "files", "--jq", ".files[].path"]:
                    # A newer PR head looks safe; the event head may not be.
                    print("flake.lock")
                    sys.exit(0)
                assert args == [
                    "pr", "view", "fixture-pr", "--json",
                    "baseRefOid,headRefOid", "--jq",
                    '.baseRefOid + " " + .headRefOid',
                ], args
                print(os.environ["LOCK_CURRENT_BASE"] + " "
                      + os.environ["LOCK_CURRENT_HEAD"])
                sys.exit(0)
            if args[:2] == ["pr", "merge"]:
                pathlib.Path(
                    os.environ["LOCK_FIXTURE_DIR"], "merge.json"
                ).write_text(json.dumps(args))
                sys.exit(0)
            if args == ["api", "--method", "GET",
                        "repos/fixture/repo/compare/base...head"]:
                print((directory / "comparison.json").read_text())
                sys.exit(0)
            if args[:4] == ["api", "--method", "GET",
                            "repos/fixture/repo/actions/runs"]:
                print("999")
                sys.exit(0)
            if args == ["api", "--method", "POST",
                        "repos/fixture/repo/actions/runs/999/approve"]:
                (directory / "approved").touch()
                sys.exit(0)
            assert args[:4] == ["api", "--method", "GET",
                               "repos/fixture/repo/contents/flake.lock"], args
            assert args[4] == "-f" and args[6:] == ["--jq", ".content"], args
            assert args[5] in ("ref=base", "ref=head"), args
            source = (
                pathlib.Path(os.environ["LOCK_FIXTURE_DIR"])
                / (args[5][4:] + ".lock")
            )
        ...
        self.env.update(
            ...
            LOCK_FIXTURE_DIR=str(self.directory),
            GH_REPO="fixture/repo",
            BASE_SHA="base",
            HEAD_SHA="head",
            AUTO_MERGE_GITHUB_OWNERS="willfish\n",
            PR_URL="fixture-pr",
            LOCK_CURRENT_BASE="base",
            LOCK_CURRENT_HEAD="head",
            PR_NUMBER="123",
            HEAD_REF="update_flake_lock_action",
        )
        self.comparison = {
    ...
    def test_immutable_file_gate_allows_only_modified_lock_before_approval(
        self,
    ):
    ...
    def test_immutable_file_gate_rejects_event_changes_despite_safe_live_pr(
        self,
    ):
    ...
    def test_immutable_file_gate_rejects_mismatched_or_incomplete_comparison(
        self,
    ):
        ...
        invalid = (
            ...
            },
            {
                ...
                + [{"filename": "other", "status": "modified"}] * 299
            },
        )
    ...
    def test_valid_nested_follows_are_resolved(self):
        self.base["nodes"]["root"]["inputs"].update(
            alias=["app", "dep"], second=["alias"]
        )
    ...
    def test_unresolvable_follows_cycles_require_review(self):
        for aliases in (
            ...
            with self.subTest(aliases=aliases):
                ...
                self.base["nodes"]["app"]["inputs"]["shared"] = [
                    next(iter(aliases))
                ]
                self.head = copy.deepcopy(self.base)
                self.assert_manual()
    ...
    def test_duplicate_json_keys_are_rejected(self):
        self.head = json.dumps(self.head).replace(
    ...
    def test_workflow_executes_only_base_branch_policy(self):
        ...
        checkout = workflow.split(
            "      - name: Check out trusted lock policy\n", 1
        )[1]
        ...
        self.assertLess(
            workflow.index("Verify PR only updates flake.lock"),
            workflow.index("Check out trusted lock policy"),
        )
        self.assertLess(
            workflow.index("python3 -I scripts/check-flake-lock-update.py"),
    ...
    def test_stale_base_or_head_stops_before_merge_command(self):
        for field in ("LOCK_CURRENT_BASE", "LOCK_CURRENT_HEAD"):
            with self.subTest(field=field):
                self.env.update(
                    LOCK_CURRENT_BASE="base", LOCK_CURRENT_HEAD="head"
                )
